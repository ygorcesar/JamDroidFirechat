...
class UserRemoteDao : AnkoLogger {
    ...
    fun fetchUsers(email: String): Observable<List<User>> {
        ...
        return RxFirebaseDatabase.observeValueEvent(usersReference, DataSnapshotMapper.listOf(User::class.java))
                .map { it.filter { it.email != email }.sortedBy { it.name } }
                .map {
                    val index = it.indexOf(it.find { it.email == ConstantsFirebase.Location.CHAT_GLOBAL })
                    Collections.swap(it, index, 0)
                    it
                }.toObservable()
    }
    ...
    fun createChat(userEmail: String, friendEmail: String): String {
        val chatKey = FirebaseDatabase.getInstance().baseChatReference().push().key
    ...
    fun chatGlobalReference() = ChatReference(ConstantsFirebase.Location.CHAT_GLOBAL, ConstantsFirebase.Location.CHAT_GLOBAL)
}